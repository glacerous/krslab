...
import { LayoutDashboard, Database, CalendarDays, Zap, ShieldCheck, Sun, Moon, X, Github, ExternalLink, Cpu, Menu } from "lucide-react";
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className="fixed inset-0 bg-background/80 backdrop-blur-sm z-[90] md:hidden animate-in fade-in duration-300"
                    onClick={onClose}
                />
            )}

            <aside className={cn(
                "fixed inset-y-0 left-0 w-64 border-r border-border bg-card flex flex-col h-screen z-[100] transition-transform duration-300 ease-in-out md:sticky md:translate-x-0 overflow-hidden",
                        ...
                        return (
                            ...
                            >
                                ...
                                {item.name === "Datasources" && showOnboarding && (
                                    <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 z-[100] animate-in slide-in-from-left-2 duration-300 md:block hidden">
                                        <div className="bg-primary text-primary-foreground px-5 py-3 rounded-2xl shadow-[0_0_40px_rgba(132,204,22,0.4)] relative w-60 border border-white/20">
                                            ...
                                            <div className="flex justify-between items-start gap-4">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <p className="text-[11px] font-bold leading-relaxed">CREATE DATASOURCE FIRST</p>
                                                </div>
                                                <button onClick={handleDismiss} className="hover:bg-white/10 p-1 rounded-full transition-colors -mr-1 -mt-1">
                        ...
                        <div className="relative bg-card border border-border/50 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-500 realistic-shadow">
                            ...
                            <div className="p-8 space-y-8">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={() => setIsCreditsOpen(false)} className="p-2 hover:bg-muted rounded-full transition-soft text-muted-foreground/30 hover:text-foreground">
                                        <X className="w-4 h-4" />
                                    </button>
                                </div>
                                <div className="space-y-4">
                                    <div className="px-5 py-6 rounded-2xl bg-muted/20 border border-border/10 transition-soft hover:bg-primary/[0.03] hover:border-primary/10 text-center flex flex-col items-center">
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <a href="https://github.com/glacerous" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted/50 hover:bg-primary/10 text-[10px] font-bold text-muted-foreground hover:text-primary transition-soft group/az">
                                    ...
                                    <div className="p-5 rounded-2xl bg-muted/5 border border-border/5 transition-soft hover:bg-muted/10">
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-2.5 pl-1">
                                                <p className="text-[13px] font-bold text-foreground leading-snug">Scheduling engine adapted from <span className="text-primary italic">KRSPlan Engine</span></p>
                                                <div className="space-y-2">
                                                    <a href="https://github.com/farhanisty/krsplan-engine" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/40 hover:bg-primary/10 text-[11px] font-bold text-muted-foreground hover:text-primary transition-soft group/link">
                                                        Original author: @farhanisty
                                                        <ExternalLink className="w-3 h-3 opacity-30 group-hover/link:opacity-100 transition-opacity" />
                                                    </a>
                                                    <p className="text-[9px] text-muted-foreground/40 font-bold uppercase tracking-wider pl-1">No license was specified in the original repository.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="pt-2 flex flex-col items-center">
                                    <div className="px-4 py-2 rounded-full bg-muted/10 border border-border/5 mb-4">
                                        <p className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] text-center italic">Adapted and extended for educational/experimental use.</p>
                                    </div>
